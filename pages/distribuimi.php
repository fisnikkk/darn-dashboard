...
try {
    $distDataFpVals = $db->query("SELECT DISTINCT CAST(data_e_fletepageses AS CHAR) AS d FROM distribuimi WHERE data_e_fletepageses IS NOT NULL AND YEAR(data_e_fletepageses) > 0 ORDER BY d")->fetchAll(PDO::FETCH_COLUMN);
} catch (Exception $e) {
    $distDataFpVals = [];
}
